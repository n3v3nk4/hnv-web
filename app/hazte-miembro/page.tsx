...
export default function HazteMiembro() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('members').insert([{ ...formData, status: 'pending' }])
    if (!error) {
      setSuccess(true)
  ...
  return (
    <div className="container mx-auto py-16 px-4">
      <div className="max-w-2xl mx-auto">
        <h1 className="text-4xl font-bold text-center mb-4">HAZTE MIEMBRO 🤝</h1>
        <p className="text-center text-gray-600 mb-10">Forma parte de la comunidad HNV</p>
        
        <div className="bg-white p-6 rounded-2xl shadow-lg mb-8">
          <p className="text-gray-700">Ser miembro de HNV significa formar parte de una comunidad que busca crecer, aprender, ayudarse y construir nuevas oportunidades.</p>
          <p className="text-gray-700 mt-2">Además de contribuir al desarrollo de nuestros programas, los miembros pueden acceder a beneficios básicos de HNV.</p>
        </div>

        <div className="bg-white p-6 rounded-2xl shadow-lg mb-8">
          <h2 className="text-2xl font-bold mb-4">Membresías mensuales</h2>
          <div className="space-y-2">
            <p>🌱 <strong>Plan Semilla</strong> — $5.000 CLP/mes</p>
            <p>🌿 <strong>Plan Comunidad</strong> — $15.000 CLP/mes</p>
            <p>⭐ <strong>Plan Desarrollo</strong> — $20.000 CLP/mes</p>
            <p>👑 <strong>Plan Visionario</strong> — $100.000 CLP/mes</p>
          </div>
          <p className="text-gray-600 mt-4 text-sm">Cada aporte contribuye al fortalecimiento de los programas, actividades y proyectos de Fundación HNV.</p>
        </div>

        <div className="bg-white p-6 rounded-2xl shadow-lg mb-8">
          <h2 className="text-2xl font-bold mb-4">🎁 Beneficios para miembros</h2>
          <h3 className="font-semibold mb-2">🧠 Masterclass gratuita</h3>
          <p className="text-gray-700 mb-4">Acceso a la Masterclass HNV de Desarrollo Personal y Resolución de Problemas, con temas como:</p>
          ...
          <h3 className="font-semibold mb-2">💆 Masaje relajante gratuito</h3>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-white p-8 rounded-3xl shadow-2xl border border-gray-100">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input type="date" name="birth_date" onChange={handleChange} className="w-full p-4 rounded-xl border-2 border-gray-200 focus:border-red-500 focus:outline-none" />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Nacionalidad</label>
              <input type="text" name="nationality" onChange={handleChange} className="w-full p-4 rounded-xl border-2 border-gray-200 focus:border-red-500 focus:outline-none" />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">País</label>
              <input type="text" name="country" onChange={handleChange} className="w-full p-4 rounded-xl border-2 border-gray-200 focus:border-red-500 focus:outline-none" />
            </div>
            <div>
          ...
          <div>
            ...
            <select name="plan_type" onChange={handleChange} className="w-full p-4 rounded-xl border-2 border-gray-200 focus:border-red-500 focus:outline-none">
